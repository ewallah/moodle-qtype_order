...
/**
 * Behat steps definitions for the order question type.
 *
 * @package   qtype_order
 * @category  test
...
/**
 * Steps definitions related with the order question type.
 *
 * @copyright 2018 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_qtype_order extends behat_base {
    ...
    protected function item_xpath_by_lable($label) {
        return '//li[@contains(normalize-space(.), "' . $this->escape($label) . '")]';
    }
     ...
     *
     * @Given /^I drag "(?P<label>[^"]*)" to space "(?P<position>\d+)" in the order question$/
     */
